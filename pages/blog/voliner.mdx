...
Its function is to **stabilize the _DC bus_ voltage
...
An inertia flywheel-based energy storage system (_FESS_) consists of the
...
This section briefly comments on the design process of a series of _structural
components_ necessary for the correct and _safe operation_ of the system.
_SolidWorks 3D_ modeling software was used to perform simple and visual sizing.

The design process began with the modeling of the existing components,
that is, the **inertia disk** and the **induction machine**.

![Intertia disk](/static/images/voliner/inertia_disk-700.jpg)

![Electric motor](/static/images/voliner/electric_motor-700.jpg)

Then, based on these 3D models, the **mechanical coupling between the
inertia disks (_flywheels_) and the electrical machine was designed**,
the base where the
_FESS_ will be mounted and the protective **acrylic dome** that prevents
accidental contact with the _moving parts_ of the system.
...
The **self-aligning coupling** is based on its radial
expansion by tightening a series of bolts located on one of its faces.
When the expansion occurs, the coupling exerts pressure on the shaft
and insert, achieving the desired fastening.

![BLK 130](/static/images/voliner/BLK130-700.jpg)

By joining all the pieces, we get a rigid **flywheel composed of two inertia
...
The design of the **mounting base** serves as a _support_ and raises the machine
so that the flywheel can _rotate freely_. It was intended that it be low
and wide to achieve _great stability_.

> The dimensions of the frame
> were chosen so that there is enough space to fit flywheels in the front of
> the engine and in the rear, allowing a possible expansion of the system in
> a future upgrade.
...
_rotating parts_ of the system while providing a _mounting place
for the encoder_. For its creation, an acrylic plate was used,
made with a CNC laser cutter.
