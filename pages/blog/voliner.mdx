...
oportunity to share my _final year university project_.
...
- Built a full flywheel energy storage system
- Mechanical assembly
- 3-phase AC-DC bidirectional electronic converter
- Software for the control system and bidirectional AC-DC conversion
- Creation of a control strategy to stabilize voltage fluctuations
- Ipmlemented active voltage regulation, start-up, shutdown and emergency subsystems
- Graphical GUI with osciloscope-like plots and PID tuning tools
- Commisioning rutine
...
it needs energy storage. The devices to achieve it can be divided in
in the following cattegories.
...
> Since the whole system is connected by a DC bus, we can assosiate
...
and FESS is a solution that combines the best of both worlds!
...
**improving the overall operation of the REI**.
...
machine can work as a _generator_ or a _motor_ in following situations:

- **Motor:** Increase it's _kinetic energy_ by accelerating the rotating element
  taking _electric energy_ from the grid.
- **Generator:** Decrease it's _kinetic energy_ by slowing down the rotating element
...
_three-phase rectifier_**. It is based on the
_Mitsubishi PS21A79_ integrated circuit that encapsulates
...
That proposes a way to eliminate the medium and high frequency currents of a _Fuel
cell_ using a _bank of supercapacitors_ that deliver and absorb energy
in a controlled manner. For this variant **instead of the _fuel cell_ a _battery bank_
...
- the **isq** current setpoint can be generated by two different
  ...
  - During **normal operation** the isq reference is generated
    ...
    quadrature current).
  - For **complementary system operation** (boot, shutdown
...
The results obtained in each test are compacted into a figure showing:
...
4. The iFW currentflank and its slow extinction results in a smooth
...
Let's analyse now the **system response to a 4A load pulse of
300ms duration**. It can b the FESS does not completely eliminate the
...
- The **FESS has a limitation in its _system response time_**.
  ...
  the induction machine currents_ found in the inner most internal control
  loop and by the _control structure itself_. **Adding capacitors is an efffective
...
- The broad **DC bus voltage range** (240V-300V) introduced an additional
  degree of _complexity_, because the _dynamics_ of the system depend on the
  value of that voltage. As a result, _PID regulators_ had to be adapted to
  ...
  regulators to act, which results in a _limitation of the incoming or outgoing
  power of the system_ (rembember that P=VxI).
- The _self-discharge_ of the constructed FESS is much _greater_ than that
  ...
  significantly impact system dynamics_, so the **results obtained are valid
