...
representations of said components.

The design process began with the modeling of the existing components,
that is, the **inertia disk** (two were used for the assembly)
...
The **self-aligning coupling** is adjusted with its _radial
...
height to achieve _great stability_.
...
> a future upgrade (since the fan in the motor back can be replaced with another
> inertia disk).
